...
* **Programmability and covenants**: each individual output (UTXO), and the transaction as a whole, carries **validation scripts** — an immutable part of the data they sit in. A script defines an enforced logical relationship, a **constraint**, between parts of the transaction data.
  ...
  A chained account is not a special kind of object built into the node. It **is** a covenant — the `chain` constraint — and that constraint is one formula among many in the shared [ledger definition library](ledgerdocs/library.md), the set of constraint formulas every node agrees on and identifies by its hash. Any UTXO may carry it, exactly as it may carry a signature lock or a time lock. The same holds for everything named above: what makes an output a sequencer, a delegation or a foundry is which library formulas it carries. The wire format reserves a fixed position in the output tuple for several of them, but they remain constraint formulas evaluated like any other — not object types implemented inside the node.
